  ...
  return (
    <Modal maxWidth="max-w-3xl" onClose={onClose}>
      <header className="flex shrink-0 items-start justify-between gap-4 border-b border-line p-5">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            {fackEnabled ? `FACK ${used} / ${capacity} used` : used > 0 ? "1 item assigned" : "Empty slot"}
          </p>
        </div>
      ...
      <div className="flex-1 space-y-5 overflow-y-auto p-5">
        ...
        {canEdit && !hasFreeCapacity ? (
          <p className="rounded-lg border border-dashed border-line p-4 text-center text-sm text-slate-400">This slot is full.</p>
        ) : null}
        {!canEdit && used === 0 ? (
...
function FackConfig({ capacity, enabled, onSaved, slotId, usedCount, warehouseId }: {
  ...
  async function save() {
    ...
    if (draftEnabled && (!Number.isInteger(parsed) || parsed < 1)) { setError("Enter a FACK count of 1 or more."); return; }
    if (parsed < usedCount) { setError(`This slot already holds ${usedCount} item(s).`); return; }
    setSaving(true);
    setError(null);
    try {
      ...
      setError(err instanceof Error ? err.message : "Could not save FACK settings.");
    } finally {
      setSaving(false);
    }
  }
  ...
  if (!open) {
    return (
      <button className="inline-flex items-center gap-2 rounded-md border border-line bg-white/[0.03] px-3 py-2 text-xs font-semibold text-slate-300 hover:border-accent hover:text-accent" onClick={() => setOpen(true)} type="button">
        <Settings2 size={14} /> FACK settings {enabled ? `(${capacity} cells)` : "(off)"}
      </button>
    );
  }
  ...
  return (
    <section className="space-y-3 rounded-lg border border-accent/30 bg-accent/[0.04] p-4">
      <h3 className="text-sm font-semibold uppercase tracking-[0.14em] text-slate-300">FACK settings</h3>
      <label className="flex items-center justify-between gap-3 text-sm text-slate-200">
        Divide this slot into FACK cells
      ...
      {draftEnabled ? (
        <label className="block text-sm text-slate-300">
          Number of FACK cells (max different items)
      ...
      <div className="flex justify-end gap-2">
        <button className="rounded-md border border-line px-3 py-1.5 text-sm font-semibold text-slate-200" onClick={() => setOpen(false)} type="button">Cancel</button>
        <button className="rounded-md bg-accent px-3 py-1.5 text-sm font-semibold text-slate-950 disabled:opacity-50" disabled={saving} onClick={() => void save()} type="button">{saving ? "Saving..." : "Save"}</button>
      </div>
    </section>
  );
}
        ...
        {panel.assignments.map((assignment) => (
          <div className="flex items-center justify-between gap-3 rounded-lg border border-emerald-400/30 bg-emerald-500/10 p-4" key={assignment.id}>
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-slate-500">{placementText(assignment.stockBalance.location?.code, assignment.fackNumber ?? assignment.stockBalance.compartment)}</p>
            </div>
            {canEdit ? (
  ...
  return (
    <section>
      ...
      <div className="mb-3 flex items-center gap-2">
        <span className="text-xs font-semibold uppercase text-slate-500">Container</span>
        <ToggleButton active={container === "pallet"} icon={<Package size={14} />} label="Pallet" onClick={() => setContainer("pallet")} />
        <ToggleButton active={container === "box"} icon={<Boxes size={14} />} label="Box" onClick={() => setContainer("box")} />
      </div>
