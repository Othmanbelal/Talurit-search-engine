...
export function StockRowMovementModal({
  ...
  async function submit() {
    setError(null);
    if (quantity > activeRow.quantity) return setError(`Only ${activeRow.quantity} ${activeRow.unit} available.`);
    if (mode === "use_in" && !cardId) return setError("Select a Used In card.");
    if (mode === "use_in" && selectedCard?.spots.length && spotIds.length !== quantity) {
      return setError(`Select ${quantity} empty spot${quantity === 1 ? "" : "s"}.`);
    }
    setIsSaving(true);
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : "Movement failed");
    } finally {
      setIsSaving(false);
    }
  }
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4">
      <section className="w-full max-w-xl rounded-lg border border-line bg-slate-950 p-5 shadow-2xl">
        <header className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-accent">Take out / Use in</p>
            <h2 className="mt-2 text-xl font-semibold text-white">{activeRow.item.name}</h2>
            <p className="mt-1 text-sm text-slate-400">Available: {activeRow.quantity} {activeRow.unit}</p>
          </div>
...
function CardFields({ cardId, cards, emptySpots, quantity, selected, setCardId, setSelected }: {
  ...
  return (
    <>
      <SelectField label="Used In card" onChange={(value) => { setCardId(value); setSelected([]); }} options={cards.map((card) => ({ label: card.name, value: card.id }))} value={cardId} />
