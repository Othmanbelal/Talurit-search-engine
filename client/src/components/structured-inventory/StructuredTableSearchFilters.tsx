    ...
    <form className="rounded-lg border border-line bg-white/[0.04] p-3" onSubmit={onSearch}>
      <div className="grid gap-2 xl:grid-cols-[minmax(220px,1fr)_210px_210px_auto_auto]">
        <input
          ...
          value={search}
        />
        <Select label={t("search.allItems")} onChange={(v) => onFilters({ ...filters, itemName: v })} options={options.itemNames} value={filters.itemName ?? ""} />
        <Select label={t("search.allManufacturers")} onChange={(v) => onFilters({ ...filters, manufacturerName: v })} options={options.manufacturers} value={filters.manufacturerName ?? ""} />
        {(visibleAttributes.length > 0 || canManage) ? (
          <button
            ...
            type="button"
          >
            <Filter size={16} /> {t("search.attributes")} {activeAttributeCount > 0 ? `(${activeAttributeCount})` : ""}
          </button>
        ) : null}
      ...
      <div className="mt-2 flex justify-end">
        <button className="text-xs font-semibold text-slate-400 hover:text-white" onClick={onReset} type="button">{t("search.resetFilters")}</button>
      </div>
    </form>
  );
}
    ...
    return (
      <section className="mt-3 rounded-md border border-accent/30 bg-accent/5 p-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-white">{t("search.configureAccessTitle")}</p>
            <p className="text-xs text-slate-400">{t("search.configureAccessDesc")}</p>
          </div>
          <div className="flex gap-2">
            ...
            <button
              ...
              type="button"
            >
              <Check size={12} /> {saving ? t("search.saving") : t("search.save")}
            </button>
          </div>
        </div>
        ...
        {effectiveDraft !== null && (
          <div className="space-y-1.5">
            {allAttributes.length === 0 && (
              <p className="text-xs text-slate-500">{t("columns.noAttributesYet")}</p>
            )}
  ...
  return (
    <section className="mt-3 rounded-md border border-line bg-slate-950/35 p-3">
      <div className="flex flex-col justify-between gap-2 md:flex-row md:items-center">
        <div>
          <p className="text-sm font-semibold text-white">{t("search.attributeFilters")}</p>
          <p className="text-xs text-slate-400">{t("search.attributeFiltersDesc")}</p>
        </div>
        <div className="flex items-center gap-2">
          {canManage && onSaveAllowedAttributes ? (
            <button
              ...
              onClick={openConfig}
              title={t("search.configureAccess")}
              type="button"
            >
              <Settings2 size={14} /> {t("search.configureAccess")}
            </button>
          ) : null}
          <button
            ...
            type="button"
          >
            <Plus size={15} /> {t("search.addFilter")}
          </button>
        </div>
      </div>
      {options.attributes.length === 0 ? <p className="mt-3 text-sm text-slate-500">{t("search.noAttributesSaved")}</p> : null}
  ...
  return (
    <div className="grid gap-2 md:grid-cols-[minmax(180px,260px)_1fr_auto]">
      <select
        ...
        value={filter.name}
      >
        <option value="">{t("search.chooseAttribute")}</option>
      ...
      <select
        ...
        value={filter.value}
      >
        <option value="">{t("search.chooseValue")}</option>
        {(attribute?.values ?? []).map((v) => <option key={v} value={v}>{v}</option>)}
      </select>
      <button className="inline-flex h-10 w-10 items-center justify-center rounded-md border border-red-400/40 bg-red-500/10 text-red-100 hover:bg-red-500/20" onClick={onRemove} title={t("search.remove")} type="button">
        <X size={16} />
      </button>
    </div>
  );
