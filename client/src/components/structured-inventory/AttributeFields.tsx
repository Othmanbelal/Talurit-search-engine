...
export function AttributeFields({
  ...
  function remove(index: number) {
    if (!window.confirm("Remove this additional column value?")) return;
    ...
    <section className="space-y-3 rounded-lg border border-line bg-white/[0.03] p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400">These are saved as item attributes in the database.</p>
        </div>
      ...
      {attributes.length === 0 ? <p className="text-sm text-slate-500">No additional columns added.</p> : null}
      <div className="space-y-2">
        {attributes.map((attribute, index) => (
          <div className="grid gap-2 md:grid-cols-[1fr_1fr_120px_40px]" key={index}>
            <Field label="Column name" onChange={(value) => update(index, { name: value })} value={attribute.name} />
            <Field label="Value" onChange={(value) => update(index, { rawValue: value })} value={attribute.rawValue ?? ""} />
            <Field label="Unit" onChange={(value) => update(index, { unit: value })} value={attribute.unit ?? ""} />
            <button className="mt-6 inline-flex h-10 w-10 items-center justify-center rounded-md border border-red-400/40 bg-red-500/10 text-red-100" onClick={() => remove(index)} title="Remove" type="button">
              <Trash2 size={15} />
            </button>
          </div>
        ))}
