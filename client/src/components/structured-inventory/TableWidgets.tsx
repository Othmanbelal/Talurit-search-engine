...
export function TableWidgets({ rows, table }: { rows: StructuredStockRowsResponse | null; table: StructuredInventoryTable | null }) {
  ...
  const items = [];
  if (settings.widgets.itemCount) items.push({ label: t("stats.items"), value: rows.stats.itemCount });
  if (settings.widgets.balance) items.push({ label: t("stats.inventoryBalance"), value: balance(rows.stats.balance, rows.stats.currency) });
