...
export function MachinesPage() {
  ...
  async function handleCreate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setActionError(createError instanceof Error ? createError.message : "Machine could not be added");
    }
  }

  async function handleDelete(machine: MachineWithSlotCount) {
    const confirmed = window.confirm(
      `Remove ${machine.name}? Inventory tools will stay in the database but will be unassigned from this machine.`,
    );
    ...
    try {
      ...
      setActionError(deleteError instanceof Error ? deleteError.message : "Machine could not be removed");
    }
  }
    ...
    <div className="mx-auto max-w-7xl space-y-5">
      <header className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div>
          ...
          <h1 className="mt-3 text-3xl font-semibold text-white md:text-4xl">Machines</h1>
        </div>
